...
bool subscrpg_animate(byte from, byte to, SubscrTransition const& transition, ZCSubscreen& parent)
{
	...
	{
		subscr_pg_bmp1 = zc_create_bitmap(SUBSCR_ACTIVE_WIDTH,SUBSCR_ACTIVE_HEIGHT);
		subscr_pg_bmp2 = zc_create_bitmap(SUBSCR_ACTIVE_WIDTH,SUBSCR_ACTIVE_HEIGHT);
		subscr_pg_subbmp = zc_create_bitmap(SUBSCR_ACTIVE_WIDTH,SUBSCR_ACTIVE_HEIGHT);
	}
	return true;
}
...
void SW_Selector::draw(BITMAP* dest, int32_t xofs, int32_t yofs, SubscrPage& page) const
{
	...
	BITMAP* tmpbmp = zc_create_bitmap(sw,sh);
