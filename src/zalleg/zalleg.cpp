...
// for those. Without a vendor id, bytes 4-15 hold the device name, and byte 14
// is a signature only if the backend set one; that can't be told apart from a
// name character, so such GUIDs are skipped (e.g. a virtual joystick created
// without a vendor id reads as generic).
const char* get_joystick_backend(const ALLEGRO_JOYSTICK_GUID& guid)
{
	if (!guid.val[4] && !guid.val[5])
		return nullptr;
