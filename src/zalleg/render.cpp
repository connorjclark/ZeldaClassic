...
// Bumped by render_tree_draw_and_flip's periodic safety refresh; a LegacyBitmapRTI
// content-hash input, so bumping it forces every legacy bitmap to re-convert.
...
// any draw path that forgot to mark itself dirty (see render_mark_dirty), turning a subtle
// stale-window bug into a logged error.
//
...
static void render_verify(RenderTreeItem* rti, ALLEGRO_COLOR clear_color, bool presented)
{
	...
	if (!verify_bitmap)
	{
		...
		if (!verify_bitmap || !baseline_bitmap)
			return;
	}
...
{
	// Once a second, force the legacy bitmaps to re-convert and a fresh frame to be
	// presented even when nothing appears to have changed. This bounds the damage of any
	// missed invalidation - an untracked draw path, or a hash blind spot - to a second of
	// lag instead of a permanently stale window. Bumping the refresh counter changes
	// every LegacyBitmapRTI content hash (it is a hash input), which re-converts them
	// and marks the tree dirty.
	static int frames_since_flip;
	bool force_refresh = ++frames_since_flip >= 60;
	if (force_refresh)
...
{
	static bool value = get_flag_bool("-no-render-hash").value_or(false);
	return value;
}
...
void LegacyBitmapRTI::render(bool size_changed)
{
	if (bitmap && a4_bitmap && (!a4_bitmap_rendered_once || !freeze))
	{
		...
		auto time_now = [&]() {
			return timings ? std::chrono::steady_clock::now().time_since_epoch().count() : 0;
		};
