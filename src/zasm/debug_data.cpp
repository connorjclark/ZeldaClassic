...
std::vector<std::string> split_identifier(const std::string& str)
{
...
DebugData::ResolveResult DebugData::resolveEntity(const std::string& identifier, const DebugScope* current_scope) const
{
	...
	auto find_member = [&](int32_t parent_idx, const std::string& name) -> ResolveResult 
	{
		ResolveResult res;
		std::vector<int32_t> search_queue = { parent_idx };
...
std::vector<const DebugScope*> DebugData::resolveFunctions(const std::string& identifier, const DebugScope* current_scope) const
{
	...
	auto collect_functions_recursive = [&](int32_t start_idx, const std::string& name) 
	{
...
std::string DebugData::internalToStringForDebugging() const
{
	...
	std::function<void(int32_t, int)> print_scope = 
		[&](int32_t scope_idx, int indent) 
	{
