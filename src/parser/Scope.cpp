...
void Scope::initFunctionBinding(Function* fn, CompileErrorHandler* handler)
{
	...
	if (parsed_comment.contains_annotation("delete"))
	{
	...
	for (auto alias : parsed_comment.get_multi_annotation("alias"))
	{
	...
	for (auto alias : parsed_comment.get_multi_annotation("deprecated_alias"))
	{
		auto alias_fn = new Function();
		alias_fn->name = alias;
		alias_fn->alias(fn);
	...
	if (parsed_comment.contains_annotation("exit"))
		fn->setFlag(FUNCFLAG_EXITS|FUNCFLAG_NEVER_RETURN);
	if (auto depr_message = parsed_comment.get_annotation("deprecated"))
	{
		fn->setFlag(FUNCFLAG_DEPRECATED);
		fn->setInfo(*depr_message);
	}
	if (parsed_comment.contains_annotation("reassign_ptr"))
		fn->setIntFlag(IFUNCFLAG_REASSIGNPTR);

	if (auto vargs = parsed_comment.get_annotation("vargs"))
	{
	...
	if (auto zasm = parsed_comment.get_annotation("zasm"))
