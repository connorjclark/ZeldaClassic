...
void RegBaseVisitor::handle_data_decl_registry(ASTDataDecl& host)
{
	...
	if (list && list->was_exported)
	{
		...
		// scaled by 10000) or the 'long' convention (raw values). The editor
		// stores whichever raw value was named, and the script reads it through
		// the variable's own type, so a mismatch is silently off by a factor of
		// 10000. (bool exports were already rejected above.)
