...
ParsedComment::ParsedComment(const std::string& comment)
{
	...
	for (auto& line : lines)
	{
		...
		{
			if (!line.empty())
				annotations.back().second += "\n" + line;
			continue;
		}

		annotations.emplace_back(current_key, line);
	}
...
bool ParsedComment::contains_annotation(const std::string& key) const
{
...
std::optional<std::string> ParsedComment::get_annotation(const std::string& key) const
{
...
std::vector<std::string> ParsedComment::get_multi_annotation(const std::string& key) const
{
