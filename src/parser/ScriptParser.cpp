...
std::shared_ptr<ASTFile> ScriptParser::parse_from_root(std::string entry_filename, std::string metadata_orig_path, std::string metadata_tmp_path, bool import_bindings)
{
	...
	// 2. The binding files (and all children, dfs preordering)
	// 3. The entry file's children, dfs preordering
	//
	// dfs preordering - fancy way to say: when an import is processed parse it and all its children before
	// processing the next import.
