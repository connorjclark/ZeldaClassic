...
void RegistrationVisitor::initInternalVar(ASTDataDeclList* var)
{
	...
	{
		if (parsed_comment.contains_annotation("zasm_var") && parsed_comment.contains_annotation("zasm_internal_array"))
		{
		...
		if (user_class && !parsed_comment.contains_annotation("zasm_var") && !parsed_comment.contains_annotation("zasm_internal_array"))
		{
		...
		int fn_value;
		if (auto zasm_var = parsed_comment.get_annotation("zasm_var"))
		{
		...
		else if (auto zasm_internal_arr = parsed_comment.get_annotation("zasm_internal_array"))
		{
			try {
		...
		bool is_internal_arr = parsed_comment.contains_annotation("zasm_internal_array");
		bool is_arr = ty.isArray();
		auto var_type = is_internal_arr ? &ty : ty.baseType(*scope, nullptr);
		auto deprecated = parsed_comment.get_annotation("deprecated");
		...
		if (auto deprecated_getter = parsed_comment.get_annotation("deprecated_getter"))
		{
			if (is_arr || is_internal_arr)
			{
...
void RegistrationVisitor::caseClass(ASTClass& host, void* param)
{
	...
	if (auto parent_class_name = parsed_comment.get_annotation("extends"))
	{
	...
	if (auto zasm_ref = parsed_comment.get_annotation("zasm_ref"))
	{
