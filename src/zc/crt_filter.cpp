...
// the source bitmap. The HLSL ones pair with allegro's default `vs_main`/`VS_OUTPUT`.
...
void main()
{
    ...
    float scan_weight = 1.0 - pow(cos(varying_texcoord.y * 2.0 * PI * u_source_size.y) * 0.5 + 0.5, scan_beam) * SCANLINE_STRENGTH;
...
float4 ps_main(VS_OUTPUT Input) : COLOR0
{
    ...
    float scan_bright = clamp(bright, SCANLINE_BRIGHT_MIN, SCANLINE_BRIGHT_MAX);
    float scan_beam   = clamp(bright * SCANLINE_BEAM_WIDTH_MAX, SCANLINE_BEAM_WIDTH_MIN, SCANLINE_BEAM_WIDTH_MAX);
    float scan_weight = 1.0 - pow(cos(co.y * 2.0 * PI * u_source_size.y) * 0.5 + 0.5, scan_beam) * SCANLINE_STRENGTH;
