...
static std::optional<JittedFunction> compile_function(zasm_script* script, JittedScript* j_script, const ZasmFunction& fn)
{
	...
	// - ebx: int32_t sp
	{
		...
		// Note: the below code does the same, but uses virtual registers instead.
		// I've found that asmjit's binpack register allocator takes too long to run,
		// so I replaced it with a linear allocator. However, a linear allocator works really
		// bad for registers used throughout the entire function, so it's better to explicitly assign
		// them like above.
	...
	// If needed, jump to the resume address.
	{
		...
		cc.jz(L_normal_entry); // If it's zero, this is a normal function start. Jump past the resume logic.
	
		//cc.mov(x86::ptr(state.ptrCtx, offsetof(JittedExecutionContext, resume_address)), 0); // TODO ! needed?
