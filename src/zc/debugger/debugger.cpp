// TODO ! profile debugger gui
// TODO ! SCOPE_FLAG_HIDDEN/SCOPE_FLAG_INTERNAL/SCOPE_FLAG_DEPRECATED is not used for anything
...
void Debugger::SetState(State new_state)
{
	...
	for (auto& data : scriptEngineDatas | std::views::values)
	{
		...
		std::string script_name = FFCore.GetScriptName(data.script_type, data.script_num);
		std::string type_name = GetScriptTypeName(data.script_type);
		std::string name = fmt::format("{} ({})", script_name, type_name);
		active_scripts.push_back(ActiveScript{.name = name, .data = &data});
	}
...
expected<DebugValue, std::string> Debugger::Evaluate(std::string expression)
{
	...
	ExpressionEvaluator eval{zasm_debug_data, selected_scope, vm};
...
std::string Debugger::ValueToStringSummary(DebugValue value)
{
	...
	if (type->isArray(zasm_debug_data))
	{
		...
		if (am.invalid()) return "null (Array)";

		return fmt::format("Array[{}]", am.size());
	}
