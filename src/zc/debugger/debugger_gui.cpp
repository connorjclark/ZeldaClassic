	...
	if (ImGui::CollapsingHeader("Breakpoints", nullptr, ImGuiTreeNodeFlags_DefaultOpen | ImGuiTreeNodeFlags_AllowOverlap))
	{
		...
		else
		{
			for (int i = 0; i < breaks.size(); ++i)
			{
				...
				if (breakpoint.type == Breakpoint::Type::Normal && ImGui::IsItemHovered())
				{
					const std::string& path = breakpoint.source_file->path;
			...
			for (int i = 0; i < debugger->console_logs.size(); ++i)
			{
				...
				else
				{
					...
					if (has_stack)
					{
						...
						is_open = ImGui::TreeNodeEx((void*)(intptr_t)i, flags, "");
...
void Debugger::InitGui()
{
	...
	// Keep the current display as the active one, so that when the main engine code creates bitmaps
	// it is associated with that display, not the debugger display (which may be closed by the
	// user).
	if (all_get_display())
