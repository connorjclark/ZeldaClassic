...
void do_npc_stopbgsfx()
{
	if(GuyH::loadNPC(GET_GUYREF) == SH::_NoError)
	{
		GuyH::getNPC()->stop_bgsfx(GuyH::getNPCIndex(GET_GUYREF));
	}
}
...
void do_npc_delete()
{
	if(GuyH::loadNPC(GET_GUYREF) == SH::_NoError)
	{
		auto ind = GuyH::getNPCIndex(GET_GUYREF);
...
void get_npcdata_initd_label(const bool v)
{
	...
	if(ArrayH::setArray(arrayptr, string(guysbuf[GET_NPCDATAREF].initD_label[init_d_index])) == SH::_Overflow)
...
std::optional<int32_t> npc_get_register(int32_t reg)
{
	...
	switch (reg)
	{
		...
		case NPCSCRIPT:
		{
			...
			{
				//enemy *e = (enemy*)guys.spr(GET_GUYREF);
	...
	switch (reg)
	{
		...
		case NPCBGSFX:
			if(GuyH::loadNPC(GET_GUYREF) == SH::_NoError)
			{
				...
				if(en->bgsfx != newSFX)
				{
					en->stop_bgsfx(GuyH::getNPCIndex(GET_GUYREF));
					cont_sfx(newSFX);
					en->bgsfx = newSFX;
				}
			}
