...
void zplayer_handle_commands()
{
	...
	if (test_zc_arg)
	{
		...
		extern TestResults test_arrays(bool);
		if (!run_tests(test_arrays, "test_arrays", verbose)) success = false;
		...
		extern TestResults test_title_standalone(bool);
		if (!run_tests(test_title_standalone, "test_title_standalone", verbose)) success = false;
