...
dHammerSmack::dHammerSmack(zfix X,zfix Y,int32_t Id,int32_t Clk, int32_t wpnSpr) : decoration(X,Y,Id,Clk)
{
	...
	wpnid=current_itemdata(itype_hammer).wpn2;
}
...
dHover::dHover(zfix X,zfix Y,int32_t Id,int32_t Clk, int32_t wpnSpr) : decoration(X,Y,Id,Clk)
{
	id=Id;
	clk=Clk;
	wpnid = current_itemdata(itype_hoverboots).wpn;
}
...
void dDivineProtectionShield::realdraw(BITMAP *dest, int32_t draw_what)
{
	...
	int32_t fb=(misc==0?
	        (current_itemdata(itype_divineprotection).wpn5 ?
	         current_itemdata(itype_divineprotection).wpn5 : (byte) iwDivineProtectionShieldFront) :
	            (current_itemdata(itype_divineprotection).wpn10 ?
	             current_itemdata(itype_divineprotection).wpn10 : (byte) iwDivineProtectionShieldBack));
	int32_t t=wpnsbuf[fb].tile;
	int32_t fr=wpnsbuf[fb].frames;
	int32_t spd=wpnsbuf[fb].speed;
	cs=wpnsbuf[fb].csets&15;
	flip=0;
	bool flickering = (current_itemdata(itype_divineprotection).flags & item_flag4) != 0;
	bool translucent = (current_itemdata(itype_divineprotection).flags & item_flag3) != 0;
