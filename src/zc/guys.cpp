...
bool enemy::shadow_overpit() const
{
	// for ( int32_t q = 0; q < hxsz; ++q )
	{
		for ( int32_t q = 0; q < hit_height; ++q )
...
bool enemy::scr_walkflag(int32_t dx,int32_t dy,int32_t special, [[maybe_unused]] int32_t dir, int32_t input_x, int32_t input_y, bool kb)
{
	...
	if(!(moveflags & move_ignore_screenedge)
		&& ((input_x<(16-nb)) || (input_y<zc_max(16-yg-nb,0))
			|| (input_x >= (world_w-16+nb)) || (input_y >= (world_h-16+nb))))
		return true;
...
int enemy::summon_enemy(int minion_id, int summon_count, int limit, bool random_count)
{
	...
	{
		existing_count = summoner_children.size();
	}
...
void enemy::fix_coords(bool bound)
{
	...
	if(bound)
	{
		...
		if ( ((unsigned)(id&0xFFF)) < MAXGUYS )
		{
			x=vbound(x, 0, (( SIZEflags&OVERRIDE_TILE_WIDTH && !isflier(id) ) ? (w-((txsz-1)*16)) : w-16));
			y=vbound(y, 0,(( SIZEflags&OVERRIDE_TILE_HEIGHT && !isflier(id) ) ? (h-((txsz-1)*16)) : h-16));
		}
		else
		{
		    x=vbound(x, 0, w-16); 
...
static void remove_armos(combined_handle_t const& handle)
{
	...
	if(f == mfARMOS_ITEM || f2 == mfARMOS_ITEM)
	{
		...
		{
			additem(tx,ty,scr->catchall, (ipONETIME2 + ipBIGRANGE) | ((scr->flags3&fHOLDITEM) ? ipHOLDUP : 0) | ((scr->flags8&fITEMSECRET) ? ipSECRETS : 0));
			sfx(scr->secretsfx);
		}
	}
