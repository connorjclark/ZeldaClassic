...
//	refresh_select_screen = 0;
	...
	sprintf(zc_aboutstr,"%s, Version %s", ZC_PLAYER_NAME, getVersion().c_str());
	

	Z_title("ZC Launched: %s, v.%s",ZC_PLAYER_NAME, getVersion().c_str());
	...
	{
		Z_error_fatal("\nIncompatible version of zelda.dat.\nPlease upgrade to %s Build %d",VerStrFromHex(ZELDADAT_VERSION), ZELDADAT_BUILD);
	}
