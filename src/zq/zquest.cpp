...
MIDI *song=NULL;
BITMAP *menu1, *mapscreenbmp, *screen2, *mouse_bmp[MOUSE_BMP_MAX][4], *mouse_bmp_1x[MOUSE_BMP_MAX][4], *icon_bmp[ICON_BMP_MAX][4], *flag_bmp[16][4], *select_bmp[2], *dmapbmp_small, *dmapbmp_large;
...
void dev_output_qrs(string const& fname);
...
//  Frameskip					 = zc_get_config("zquest","frameskip",0); //todo: this is not actually supported yet.
	...
	if(!screen2 || !menu1 || !dmapbmp_large || !dmapbmp_large || !brushbmp || !brushscreen)// || !brushshadowbmp )
	{
...
static vector<string> wrap_qr_info(string info)
{
	...
	while (idx < info.size())
	{
		...
		string tmp = info.substr(idx, new_idx-idx);
...
void dev_output_qrs(string const& fname)
{
	static std::map<int, string> qr_const_names = {
