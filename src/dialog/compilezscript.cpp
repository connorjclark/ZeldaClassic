...
bool do_compile_and_slots(int assign_mode, bool delay)
{
	...
	uint32_t lastInitSize = old_init_script._zasm_script->size;
	...
	if(WarnOnInitChanged && noquick_compile)
	{
		...
		if(new_init_script._zasm_script->zasm != old_init_script._zasm_script->zasm) //Global init changed
		{
			AlertFuncDialog("Init Script Changed",
				"Either global variables, or your global script Init, have changed. ("+to_string(lastInitSize)+"->"+to_string(new_init_script._zasm_script->size)+")\n\n"
