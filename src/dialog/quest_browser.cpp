...
bool QuestBrowserDialog::pumpScan()
{
	...
	{
		statusLabel->setText(fmt::format("{:>44}",
			fmt::format("Update available: {}", new_version)));
		return true;
	}
...
std::shared_ptr<GUI::Widget> QuestBrowserDialog::view()
{
	...
	// Left-padded so the text ends at the label's (and window's) right edge,
	// with room reserved for the update-available message.
	std::string version_text = fmt::format("{:>44}", fmt::format("v{}", getVersionString()));
	int32_t version_w = text_length(dlgfont, version_text.c_str()) + pad2;
	int32_t footer_spacer = std::max(8, list_w - checkbox_w - version_w);
	...
	std::shared_ptr<GUI::Window> window = Window(
		...
		Column(
			...
			Row(
				Button(text = "&Load Quest", topMargin = 16_px, onClick = message::PICK)
			),
			Row(
				Checkbox(
				...
				Label(text = "", width = GUI::Size::pixels(footer_spacer)),
				statusLabel = Label(text = version_text)
			)
		)
	);
