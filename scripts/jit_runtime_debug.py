...
def diverges_with_hi(hi: int) -> bool:
    ...
    if roundtrip is None:
        # A run that dies before writing its roundtrip (a hard crash under this
        # JIT split) is as diverging as a value mismatch - bisect toward it
        # instead of silently counting the crash as a match.
        if rc != 0:
            print(f'-jit-run-hi {hi} crashed without a roundtrip; treating as divergence')
            return True
        raise Exception(f'-jit-run-hi {hi} passed but produced no .roundtrip file')
