...
To see what the viewport is doing while tuning these, turn on ``Cheats > Show/Hide Layer > Viewport Follow`` in the player. It draws the deadzone box (cyan) around the viewport's focus point (white cross), and the current lookahead offset as a yellow line from the player's center to the point the viewport is aiming at, with a hollow dot marking the value it is easing toward. The overlay is hidden while a script or a camera cutscene effect controls the viewport.
