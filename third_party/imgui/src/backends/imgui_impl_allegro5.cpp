...
ImGuiKey ImGui_ImplAllegro5_KeyCodeToImGuiKey(int key_code)
{
    switch (key_code)
    {
		...
		case ALLEGRO_KEY_COMMAND: return ImGuiKey_LeftSuper;
...
static void ImGui_ImplAllegro5_UpdateKeyModifiers()
{
    ...
    // local edit - fixes cmd/ctrl on mac.
    ...
    bool super_down = al_key_down(&keys, ALLEGRO_KEY_LWIN) || al_key_down(&keys, ALLEGRO_KEY_RWIN);
#ifdef __APPLE__
    super_down |= al_key_down(&keys, ALLEGRO_KEY_COMMAND);
#endif
...
static void ImGui_ImplAllegro5_UpdateMouseCursor()
{
    ...
    else
    {
		...
		case ImGuiMouseCursor_Hand:         cursor_id = ALLEGRO_SYSTEM_MOUSE_CURSOR_LINK; break; // local edit
